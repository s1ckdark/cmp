...
const NoticeDetailView = ({ id }: any) => {
  ...
  return (
    <div className={style.noticeDetailView}>
      ...
      {data.uploadedFiles && data.uploadedFiles.length > 0 && <FileDownloader data={data.uploadedFiles} pageType="view" />}
      <div className={style.btnArea}>
        <Button onClick={() => router.push(`/notice/edit/${id}`)} skin="submit">
          수정
        </Button>
        <Button onClick={() => router.back()} skin="cancel">
          목록
        </Button>
      </div>
    </div>

...
const BillingForm = ({pageType, product}:IBillingProductForm) => {
  ...
  const onSubmit = async (formData: any) => {
    ...
    const response = pageType === 'register' ? await apiBe.put(`/billing`, tmp) : await apiBe.post(`/billing`, tmp);
    if (response.status === 201 || response.status === 200) {
        const { data } = response;
        if(pageType === 'register') Toast("success", '고객사 정보가 저장 되었습니다.', ()=> goList());
        if(pageType === 'edit') Toast("success", '고객사 정보가 저장 되었습니다.', ()=> goList());
    } else {
        ...
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={`${styles.btnArea} mt-6 mx-auto`}>
              {pageType === 'register' || pageType === 'edit' ? <Button type='submit' className={styles.submitBtn} skin='submit'>{pageType === 'register' ? "등 록" : "저 장"}</Button> : <Button type='button' onClick={editMode} className={styles.submitBtn} skin='green'>수 정</Button>}
              <Button type='button' className={styles.cancelBack} onClick={goList} skin='cancel'>취 소</Button>
            </div>
          </form>
        </FormProvider>
      </>

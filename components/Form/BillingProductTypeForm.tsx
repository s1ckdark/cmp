...
const BillingProductForm = ({prodType, pageType, product}:IBillingProductForm) => {
    ...
    const { control, register, setValue, reset, watch, getValues, formState: { errors }}:any = useFormContext();
    ...
    useEffect(() => {
        ...
        if(isExist) {Toast('error', '이미 추가된 상품입니다.',()=>setModal({...modal, data: null}))}
            ...
            <table>
                ...
                <tbody>
                    {fields.map((field, index) => {
                        ...
                        return(
                            <tr key={field.id}>
                                <td><span onClick={() => window.confirm("삭제하시겠습니까?") && deleteProd(index, prodType)}>&times;</span></td>
                                ...
                                <td><input type="text" {...register(`${prodType}.${index}.prodId`, { disabled:isDisabled, required: false })} /></td>
                                <td><input type="text" {...register(`${prodType}.${index}.prodName`, { disabled:isDisabled, required: false })}/></td>
                                <td><input type="text" {...register(`${prodType}.${index}.prodDetailType`, { disabled:isDisabled, required: false })}/></td>
                                {prodType === 'billingSwList' && <td><input type="text" {...register(`${prodType}.${index}.prodDetailTypeStd`, { disabled:isDisabled, required: false })}/></td>}
                                <td><input type="number" {...register(`${prodType}.${index}.stdPrice`, { disabled:isDisabled, required: false, valueAsNumber:true })}/></td>
                                {prodType === 'billingSwList' && <td><input type="number" {...register(`${prodType}.${index}.expPrice`, { disabled:isDisabled, required: false })} /></td>}
                                <td><input type="number" {...register(`${prodType}.${index}.discountRate`, { disabled:isDisabled, required: false, valueAsNumber:true })}/></td>
                                {prodType === 'billingMspList' &&<td><input type="number" {...register(`${prodType}.${index}.qty`, { disabled:isDisabled, required: false,valueAsNumber:true})} /></td>}
                                <td><input type="number" {...register(`${prodType}.${index}.estimateuseAmount`, { disabled:isDisabled, valueAsNumber:true })} value={estimateUseAmount}/></td>
                                <td><input type="text" {...register(`${prodType}.${index}.service_start_date`, { disabled:isDisabled, required: false })}/></td>
                                <td><input type="text" {...register(`${prodType}.${index}.service_end_date`, { disabled:isDisabled, required: false })}/></td>
                                <td><input type="text" {...register(`${prodType}.${index}.comment`, { disabled:isDisabled, required: false })}/></td>
                            </tr>
                        )})}
                </tbody>
            </table>
        </div>
        { pageType !== 'view' && <div className={styles.btnArea}>
            <Button type="button" skin={"green"} onClick={() => openModal('product')}>상품추가</Button>
        </div>}
        </>
    )
}

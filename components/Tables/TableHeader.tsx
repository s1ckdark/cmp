...
export const TableHeader: React.FC<TableHeaderProps> = ({ rowType }) => {
  ...
  const types: TypesMap = {
    ...
    top10: [{ label: '전체 고객사', subHeaders: ['월', '고객ID', '고객사명', '금액(KRW)'] }], //전체고객사 추가
    support: ['지원번호', '고객사번호', '고객사명', '제목', '지원유형', '지원담당자명', '고객사담당자명', '등록자', '등록일시', '상태'],
    billingProduct: ['선택', '고객사번호', '고객사명', '구분', '상품명', '상품상세분류', '상품가격기준', '정식단가(KRW)', '노출단가(KRW)', '할인율(%)', '등록자', '등록일시'], //추가
  ...
  const sortList: any = {
    ...
    billingProduct: ['category', 'memberNo', 'memberName', 'prodName', 'prodDetailType', 'prodDetailTypeStd', 'expPrice', 'stdPrice', 'discountRate', 'regName', 'regDt'], // 고객사 자사 상품
  };
    ...
    return (
      <tr>
        {headers.map((header: any, index: number) => {
          ...
          return (
            <th key={index} className="border" rowSpan={isComplexHeader ? header.rowSpan || 1 : 1} colSpan={isComplexHeader && header.subHeaders ? header.subHeaders.length : 1} onClick={() => isSortable && toggleSortState(sortMapping[fieldKey] || fieldKey)}>
              {isComplexHeader ? header.label : header}
              {isSortable && <span className={Styles.sortToggle}>{isCurrentSortField ? (sortState.direction === 'asc' ? '▲' : '▼') : '▼'}</span>}
            </th>
          );
        })}
      </tr>
      ...
      <tr>
        {headers.flatMap((header: any) => {
          return header.subHeaders
            ? header.subHeaders.map((subHeader: string, index: number) => {
                ...
                return (
                  <th key={`${subHeader}-${index}`} className="border" onClick={() => isSortable && toggleSortState(sortMapping[subHeader] || subHeader)}>
                    {subHeader}
                    {isSortable && <span className={Styles.sortToggle}>{isCurrentSortField ? (sortState.direction === 'asc' ? '▲' : '▼') : '▼'}</span>}
                  </th>
                );

...
const FileDownloader = ({ data, type, from }: any) => {
    ...
    const goDownload = async (objectName: string, fileName: string) => {
        ...
        const response = await apiBe.post(url, data, { responseType: 'blob'}).then((response:any) => {
    ...
    const delFile = async(id:any, fileId: any, objectName: any, from:any) => {
        ...
        switch(from) {
            case 'notice':
                ...
                break;
            case 'supportboard':
                url = "/supportboard/file";
                break;
            default:
                break;
        }
        const response = await apiBe.delete(url, {data:{id: id, fileId:fileId, objectName: objectName}});
                    ...
                    {data && data.map((file: any, index: number) => {
                        return (
                            <div key={index} className={style.file}>
                                <p onClick={()=>goDownload(file.objectName, file.originName)}>{file.originName}</p>{type === 'view' ? null:<span className={style.delBtn} onClick={()=>delFile(id, file.id, file.objectName, from)}>delete</span>}
                            </div>
                        )
                    })}
                </div>

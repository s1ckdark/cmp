...
const Summary = ({ header, month }: ISummaryProps) => {
  ...
  useEffect(() => {
    ...
    const getOverview = (data: any) => {
      const result = data.reduce((acc: any, cur: any) => {
        acc.push({
          targe_month: cur.targetMonth.substr(4, 6),
          useAmount: cur.summary.useAmount,
        });
        return acc;
      });
      return result;
    ...
    const getDonutChart = (data: any) => {
      ...
      return top10.map((item: any) => ({
        // ...item,
        name: item.memberName !== null ? item.memberName : '미등록',
    ...
    const getPerMonthData = (data: any) => {
      const result = data.reduce((acc: any, cur: any) => {
        acc.push({
          x: cur.trend.current_month.substr(4, 2),
          y: cur.trend.current_totalDemandAmount,
          z: cur.trend.diff,
          a: cur.trend.growth_rate,
        });
        return acc;
      }, []);
      return result;
  ...
  if (!top10bycust || !trendMonth || !donutChart || !diffMonth) return <Loading className="loadingPage" />;
            ...
            <div className={`${styles.left} ${styles.boxSection} prevMonth flex items-center justify-center`}>
              <div className={styles.inner}>
                ...
                <div className={styles.countup}>
                  ...
                  <span>
                    <CountUpAnimation endValue={diffMonth.trend.prev_totalDemandAmount} duration={500} /> KRW
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className={styles.inner}>
                <div className={styles.label}>
                  <h2>
                    금월 네이버 전체매출
                    <span>
                      {Math.round(diffMonth.trend.growth_rate)}
                      &#37;
                    </span>
                  </h2>
                </div>
                <div className={styles.countup}>
                  <label htmlFor="totalDemandAmount">전체고객사</label>
                  <span>
                    <CountUpAnimation endValue={diffMonth.trend.current_totalDemandAmount} duration={500} /> KRW
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className={styles.trendMonth}>
            <div className={`${styles.boxSection} period_sales flex items-center justify-center flex-wrap`}>
              <div className={styles.inner}>
                ...
                <div className={styles.lineChart}>{trendMonth && <LineChart data={trendMonth} aspectRatio={16 / 5} />}</div>
